...
for folder_idx = [15, 16, 17, 18, 19]%19:numel(sub_folders)
    ...
    for idx=1:numel(files)
        fileflag(idx, 1) = ~isempty(regexp(files(idx).name, 'eye_sub[0-9][0-9][0-9][0-9]_CS_[1-3].*tsv', 'once')) + ~isempty(regexp(files(idx).name, 'eye_sub[0-9][0-9][0-9][0-9]_CS_[1-3].*csv', 'once'));
    end
    files = files(fileflag);
...
function ET_trial_data = preprocess_ET_data(ET_filepath, exp_name, CFG)
    ...
    for trial_idx = 1:n_trials
        ...
        try
            ...
            GPX = ET_event_data.GazePointX_DACSPx_;
        end
        ...
        for j = 1:numel(event_period_idx)
            try
                ...
                tmp = ET_event_data(j,:).PupilDiameterLeft_mm_;
            end
            if isnumeric(tmp)
                PDL(j) = tmp;
            else
        ...
        try
            ...
            disp(PDL);%PDL = zeros(numel(PDL), 1);
        end
        ...
        for j = 1:numel(event_period_idx)
            try
                ...
                tmp = ET_event_data(j,:).PupilDiameterRight_mm_;
            end
            if isnumeric(tmp)
                PDR(j) = tmp;
            else
        ...
        try
            ...
            disp(PDR); %PDR = zeros(numel(PDR), 1);
        end
    ...
    try
        ...
        disp('assertion sum(isnan(ET_trial_data(:))) == 0 failed')
    end
